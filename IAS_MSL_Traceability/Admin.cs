		...
		private void btnUpdate_Click(object sender, EventArgs e)
		{
			if (isInsert)
			{
				try
				{
					if (txt5.Text == "")
					{
						mySqlCommandUpdate = new MySqlCommand("UPDATE `IAS_MSL_Tb` SET MSL='"+(txt8.SelectedIndex+1)+"', BarcodeCol4='U" + txt1.Text + "', BarcodeCol6='X" + txt2.Text + "',ReleaseDate='" + txt3.Text + "',UsageStartDate='" + txt4.Text + "', `UsageEndDate`= NULL ," +
						" `MSLDate`='" + txt6.Text + "',BarcodeCol5='Q" + txt7.Text + ".0' , `Status`='" + txt8.Text + "' WHERE BarcodeCol1='" + txtBarcode.Text.Split(',')[0] + "'", this.form1.mySqlConnection);
					}
					else
					{
						mySqlCommandUpdate = new MySqlCommand("UPDATE `IAS_MSL_Tb` SET MSL='" + (txt8.SelectedIndex + 1) + "',BarcodeCol4='U" + txt1.Text + "', BarcodeCol6='X" + txt2.Text + "',ReleaseDate='" + DateTime.Parse(txt3.Text).ToString("dd.MM.yyyy") + "',`UsageStartDate`= '" + DateTime.Parse(txt4.Text).ToString("yyyy-MM-dd HH:mm:ss") + "', `UsageEndDate`= '" + DateTime.Parse(txt5.Text).ToString("yyyy-MM-dd HH:mm:ss") + "', `MSLDate`='" + txt6.Text + "',BarcodeCol5='Q" + txt7.Text + ".0' , `Status`='" + txt8.SelectedIndex + "' WHERE BarcodeCol1='" + txtBarcode.Text.Split(',')[0] + "'", this.form1.mySqlConnection);
					}
		...
		public bool printerFunction2(string barcode, string eps, string product_date, string expiration_date, string package_date, string cabine_out_date, string cabine_expiration_date, string msl, string life_time, string quantity)  //PRINTER AKSİYON
		{
			try
			{
				...
				if (!RawPrinterHelper.SendStringToPrinter(Setting.Default.printerName, test))
				{
					...
					MessageBox.Show("Printer Error1");
				}
				return true;
			}
			catch (Exception ex)
			{
				...
				MessageBox.Show("Printer Error2" + ex.ToString());
				return false;
			}

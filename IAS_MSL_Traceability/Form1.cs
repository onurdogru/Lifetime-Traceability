...
namespace IAS_MSL_Traceability
{
	public partial class Form1 : Form
	{
		...
		private void btnEkle_Click(object sender, EventArgs e)
		{
			if (barcodeControl(txtBarcodeInsert))
			{
				if (txtBarcodeInsert.Text != "")
				{
					...
					if (sqlDataReader.Read())
					{
						...
						{
							MessageBox.Show("Aynı ürün tekrar eklenemez !");
						}

					}
					else
					{
						MessageBox.Show("EPS kodu mevcut değildir !");
					}
					sqlDataReader.Close();
					timer1.Enabled = true;
				}
			...
			{
				MessageBox.Show("Lütfen doğru barkot tanıtın !");
			}
		...
		private void btnGetMLS_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show("Lütfen doğru barkot ve adet bilgisi giriniz !");
			}
